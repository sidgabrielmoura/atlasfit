  ...
  return (
    <div className="p-6 md:p-8 space-y-10 max-w-[1600px] mx-auto animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <UserStatCard title="Total Usuários" value={(snap.users || []).length.toLocaleString()} icon={UserCheck} color="bg-emerald-500/10 text-emerald-600" />
        <UserStatCard title="SuperAdmins" value={(snap.users || []).filter((u: any) => u.role === "SUPERADMIN").length.toString()} icon={ShieldAlert} color="bg-rose-500/10 text-rose-600" />
        <UserStatCard title="Atividade Recente" value={`+${recentUsersCount} novos (7d)`} icon={Activity} color="bg-primary/10 text-primary" />
      </div>
            ...
            <div className="flex max-md:flex-col items-center justify-between w-full">
              ...
              <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
                <DialogTrigger asChild>
                  <Button className="h-10 rounded-xl gap-2 max-md:w-full mt-2 font-bold bg-primary text-primary-foreground shadow-lg shadow-primary/20">
                ...
                <DialogContent>
                  ...
                  <form onSubmit={handleCreateUser} className="space-y-4 py-4">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="role">Cargo Global (Role)</Label>
                      <Select
                        value={formData.role}
                      ...
                      >
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="SUPERADMIN">Administrador (SUPERADMIN)</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    ...
                    <tr key={user.id} className="group hover:bg-secondary/20 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="size-10 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-sm uppercase">
                            {user.name?.charAt(0) || user.email?.charAt(0)}
                          </div>
                          <div className="flex flex-col">
                            <span className="text-sm font-bold leading-none">{user.name || "Sem Nome"}</span>
                      ...
                      <td className="px-6 py-4">
                        <span className={cn(
                          ...
                          user.role === "SUPERADMIN" ? "bg-rose-500/10 text-rose-600" : "bg-emerald-500/10 text-emerald-600"
                        )}>
                          {user.role}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-48 rounded-xl border-border/50">
                            ...
                            <DropdownMenuItem className="h-9 rounded-lg gap-2 cursor-pointer text-rose-600 focus:text-rose-600 focus:bg-rose-500/10">
                              <Ban className="size-3.5" />
                              <span className="text-xs font-bold">Bloquear Acesso</span>
                            </DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </td>
